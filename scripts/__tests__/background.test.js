...
describe('Background.js JD browser solver hand-off (issue #5)', () => {
  ...
  it('routes a solved token to a mydns HTTPS callback (MyJD local / Docker)', async () => {
    const mydnsCallback = 'https://127-0-0-1.mydns.jdownloader.org:3129/captcha/hcaptcha/ddownload.com?id=42';
    await send({
      action: 'jd-browser-solver-job',
      data: { jobDetails: JOB_DETAILS, callbackUrl: mydnsCallback }
    }, 31);

    await send({
      action: 'captcha-solved',
      data: { token: 'y'.repeat(40), callbackUrl: mydnsCallback, captchaId: '42' }
    }, 31);
    await new Promise(resolve => setTimeout(resolve, 20));

    expect(global.fetch).toHaveBeenCalledTimes(1);
    const [requestedUrl] = global.fetch.mock.calls[0];
    expect(requestedUrl).toBe(mydnsCallback + '&do=solve&response=' + 'y'.repeat(40));
    expect(global.chrome.tabs.sendMessage).not.toHaveBeenCalled();
  });
...
describe('Background.js remote MyJD captcha auto-open', () => {
  const JOB_DETAILS = {
    captchaType: 'hcaptcha',
    siteKey: '10000000-ffff-ffff-ffff-000000000001',
    siteKeyType: 'NORMAL',
