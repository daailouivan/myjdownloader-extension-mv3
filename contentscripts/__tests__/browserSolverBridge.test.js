...
describe('browserSolverBridge — JD browser solver hand-off (issue #5)', () => {
    ...
    it('hands off hCaptcha on MyJD local mydns HTTPS (Docker / NAS path)', () => {
        const url = 'https://127-0-0-1.mydns.jdownloader.org:3129/captcha/hcaptcha/ddownload.com?id=42';
        const [message] = run({ url, metas: Object.assign({}, HCAPTCHA_METAS, { challengeId: '42' }) });

        expect(message.action).toBe('jd-browser-solver-job');
        expect(message.data.callbackUrl).toBe(url);
        expect(message.data.jobDetails.hoster).toBe('ddownload.com');
    });

    it('also accepts localhost.mydns.jdownloader.org as loopback', () => {
        const url = 'https://localhost.mydns.jdownloader.org:3129/captcha/hcaptcha/ddownload.com?id=7';
        const sent = run({ url, metas: Object.assign({}, HCAPTCHA_METAS, { challengeId: '7' }) });

        expect(sent).toHaveLength(1);
        expect(sent[0].data.callbackUrl).toBe(url);
    });

    it('still ignores unrelated HTTPS hosts even with a captcha path', () => {
        const sent = run({ url: 'https://evil.example/captcha/hcaptcha/ddownload.com?id=7' });
